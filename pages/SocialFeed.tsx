    ...
    return (
        <a href={post.articleUrl} target="_blank" rel="noopener noreferrer" className="block mt-4 rounded-2xl overflow-hidden border border-gray-200 hover:border-purple-300 transition-all group/article bg-white shadow-sm hover:shadow-md">
            ...
            <div className={`p-5 ${post.articleImage && !imgError ? 'border-t border-gray-100' : ''}`}>
                ...
                <h3 className="text-[18px] font-bold text-gray-900 group-hover/article:text-purple-600 leading-[1.3] transition-colors">
                    {title}
                </h3>
...
export default function SocialFeed() {
  ...
  const copyRepairSql = () => {
    const sql = `-- REGESTRA NUCLEAR REPAIR (V14)
-- This version forces permissions and schema reload.
...
-- 2. Re-grant ALL permissions to ensure the API can see the changes
...
-- 3. FORCE the API to reload the schema
NOTIFY pgrst, 'reload schema';

SELECT 'Repair V14 Applied! Now use the "Project Reload" button in the app.' as result;`.trim();
  ...
  const handleForceSync = () => {
      // Nuclear programmatic refresh: Clear all caches and reload
      localStorage.clear();
  ...
  const handleSaveEdit = async (postId: string) => {
      ...
      try {
          await db.feed.updatePost(currentUser.id, postId, editingPostContent.trim());
  ...
  const isSchemaError = error?.includes('SCHEMA MISMATCH') || error?.includes('SYNC ERROR');
  ...
  return (
    <div className="max-w-xl mx-auto py-8 px-4">
                    ...
                    {previewStatus === 'failed' && (
                        <div className="p-4 flex items-center justify-between gap-3 bg-gray-50">
                            <div className="flex items-center gap-3 min-w-0">
                                ...
                                <div className="min-w-0">
                                    <p className="text-sm font-medium text-gray-900 truncate">{getDomain(articleUrl)}</p>
                                    <p className="text-xs text-gray-500 truncate">{articleUrl}</p>
                                </div>
                            </div>
        ...
        {error && (
            <div className={`mb-6 p-6 rounded-2xl border animate-slide-up shadow-sm flex flex-col gap-4 ${isSchemaError ? 'bg-amber-50 border-amber-200 text-amber-900' : 'bg-red-50 border-red-200 text-red-900'}`}>
                <div className="flex items-start gap-4">
                    {isSchemaError ? <ShieldAlert className={`w-8 h-8 flex-shrink-0 mt-0.5 text-amber-600`} /> : <AlertTriangle className="w-8 h-8 flex-shrink-0 mt-0.5 text-red-600" />}
                    <div className="flex-1">
                        <p className="font-black text-lg tracking-tight">{isSchemaError ? 'Schema Mismatch' : 'Request Failed'}</p>
                        ...
                        {isSchemaError ? (
                            <div className="mt-4 p-4 bg-white/50 rounded-xl border border-amber-200">
                                ...
                                <ol className="text-xs space-y-3 list-decimal ml-4 mb-4 font-medium">
                                    <li>Run the <b>V14 fix script</b> in your Supabase SQL Editor.</li>
                                    <li>Click <b>Force Sync & Project Reload</b> below.</li>
                                </ol>
                                <div className="flex flex-wrap gap-2">
                                    <Button size="sm" variant="default" onClick={copyRepairSql} className="h-10 px-6 rounded-xl shadow-lg flex-1">
                                        {copiedRepair ? <><ClipboardCheck className="w-4 h-4 mr-2" /> Copied!</> : <><Database className="w-4 h-4 mr-2" /> Copy Fix SQL</>}
                                    </Button>
                                    <Button size="sm" variant="outline" onClick={handleForceSync} className="h-10 px-6 rounded-xl border-amber-300 text-amber-800 bg-white hover:bg-amber-50 flex-1">
                                        <RefreshCw className="w-4 h-4 mr-2" /> Force Reload
                                    </Button>
                                </div>
                            </div>
                        ) : (
                            <div className="mt-4 flex gap-2">
                                <Button size="sm" variant="outline" onClick={fetchPosts} className="h-9 px-4 rounded-xl border-red-200 text-red-800 bg-white">
                                    <RefreshCw className="w-3.5 h-3.5 mr-2" /> Try Refreshing Feed
                                </Button>
                            </div>
                        )}
                        <button onClick={() => { setError(null); setIsPosting(false); }} className={`text-xs font-bold hover:underline mt-4 ${isSchemaError ? 'text-amber-600' : 'text-red-600'}`}>Discard Draft & Reset</button>
                    </div>
                </div>
            </div>
        )}
                ...
                {editingPostId === post.id ? (
                  <div className="mt-4">
                    <textarea
                      ...
                      className="w-full rounded-xl border border-purple-200 bg-purple-50/30 p-3 text-[15px] text-gray-800 leading-relaxed resize-none focus:outline-none focus:ring-2 focus:ring-purple-300 min-h-[80px]"
                      autoFocus
                    />
                    <div className="flex gap-2 mt-2 justify-end">
                      <button
                        onClick={() => setEditingPostId(null)}
                        className="px-4 py-1.5 rounded-full text-sm font-semibold text-gray-500 hover:bg-gray-100 transition-colors"
                      >
                        Cancel
                      </button>
                      <button
                        ...
                        className="px-4 py-1.5 rounded-full text-sm font-bold bg-purple-600 text-white hover:bg-purple-700 disabled:opacity-50 transition-colors flex items-center gap-1.5"
                      >
                      ...
                      {post.comments.map(comment => (
                        <div key={comment.id} className="flex gap-3 text-sm animate-fade-in">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-[10px] text-gray-400 mt-1 ml-3 font-bold uppercase tracking-wider">{comment.timestamp}</p>
                          </div>
                        </div>
                      ))}
                    </div>
